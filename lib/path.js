...
export function threadfieldQueue(
  ...
  const personaExcluded = new Set(persona?.treatAsWatched ?? []);
  const watchedSet = new Set(watchedIds);
  const skippedSet = new Set(skippedIds);

  const ordered = [...catalog.titles]
    .sort((a, b) => a.order - b.order)
    .filter((title) => !personaExcluded.has(title.id));
  ...
  const visibleIds = new Set([
    ...
    ...skippedIds.filter((id) => ordered.some((title) => title.id === id)),
