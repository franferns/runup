...
export function mergeRunupState(local, remote) {
  ...
  const remoteNewer = remoteEpoch > localEpoch;
  const localNewer = localEpoch > remoteEpoch;
  ...
  let budgetHours;
  if (remoteNewer) {
    personaId = base.personaId ?? null;
    budgetHours = base.budgetHours ?? null;
  } else if (localNewer || local.personaId) {
    personaId = local.personaId ?? null;
    budgetHours = local.budgetHours ?? null;
  } else {
    personaId = base.personaId ?? null;
    budgetHours = base.budgetHours ?? null;
  }
